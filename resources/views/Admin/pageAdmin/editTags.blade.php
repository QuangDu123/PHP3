...
<div class="page-content fade-in-up">
    <div class="row">
        <div class="col-md">
            <div class="ibox">
                        <h2><center>CHỈNH SỬA TAGS</center></h2>
                <div class="ibox-body">
                    <form action="#" method="POST">
                        <div class="form-group">
                            <label>Tên loại tin</label>
                            <input name="noiDung" class="form-control" type="text" value="{{$tags->noiDung}}">
                        </div>
                        <div class="form-group">
                            <label>Bài viết được gắn Tags</label>
                            <select name="idTin" class="form-control">
                                @foreach ($tin as $t)
                                <option value="{{$t->id}}" @if ($t->id == $tags->idTin) selected
                                @endif>{{$t->tieuDe}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
